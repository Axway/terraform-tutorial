...
resource "aws_instance" "bastion" {
  ...
  user_data = "${data.template_file.auto_source_global_variables.rendered}"
}

data "template_file" "auto_source_global_variables" {
  template = "${file("${path.module}/user-data/bastion-user-data.sh")}"
